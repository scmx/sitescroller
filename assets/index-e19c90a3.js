    ...
    if (this.player.state === PlayerState.jumping) {
      ...
      if (door) {
        history.pushState(
          {},
          "",
          `${"/sitescroller/"}${door.pathname}`.replace(/^\/+/, "/")
        );
        this.pathname = buildPathname(door.pathname);
        this.player.pos.x = 0;
      }
    }
  }
}
function buildPathname(pathname2, base = "/sitescroller/") {
