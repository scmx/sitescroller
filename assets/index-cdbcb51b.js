...
class Game {
  constructor(player) {
    ...
    __publicField(this, "pathname", buildPathnameGame(location.pathname));
    this.player = player;
    this.levels = Object.fromEntries(
      levelsData.map((data, index2) => [
        buildPathnameGame(data.pathname),
  ...
  update(view, deltaTime, input) {
    ...
    if (this.player.state === PlayerState.jumping) {
      ...
      if (door) {
        history.pushState({}, "", buildPathnameUrl(door.pathname));
        this.pathname = buildPathnameGame(door.pathname);
        this.player.pos.x = 0;
      }
    }
  }
}
function buildPathnameGame(pathname2, base = "/sitescroller/") {
  pathname2 = pathname2.replace(base, "");
  pathname2 = pathname2.replace(/^\/*/, "/");
  pathname2 = pathname2 === "/" ? "" : pathname2;
  window.p = pathname2;
  return pathname2;
}
function buildPathnameUrl(pathname2, base = "/sitescroller/") {
  return `${base.replace(/\/+$/, "")}${pathname2}`.replace(/^\/+/, "/");
}
